...
% cpbind(:,1) = fsolve(@(c) (1+r0)*Amat(:,1)+H(c,Ymat(:,1),w0).*Ymat(:,1).*w0-c+b,cp0(:,1),options);
% cpbind(:,2) = fsolve(@(c) (1+r0)*Amat(:,2)+H(c,Ymat(:,2),w0).*Ymat(:,2).*w0-c+b,cp0(:,2),options);
...
K_rat = mean(mean(at(:,T-100:T))/mean(yt(:,T-100:T).*ht(:,T-100:T)));
