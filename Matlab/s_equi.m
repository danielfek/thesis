function [residual,at,ct,incm] = s_equi(r0,crit,I,T,Amat,Ymat,alpha,b,delta,rho,A0,C0,pi)
% this function
...
cpbind = (1+r0) * Amat + Ymat * w0 + b;
 ...
 %cpbind(:,1) = fsolve(@(c) (1+r0)*Amat(:,1)+Ymat(:,1).*w0-c+b,cp0(:,1),options);
 %cpbind(:,2) = fsolve(@(c) (1+r0)*Amat(:,2)+Ymat(:,2).*w0-c+b,cp0(:,2),options);
...
K = mean(mean(at(:,T-1000:T)))

%L = mean(mean(yt(:,T-1000:T)))
L = 1
